	...
	return (
		<section>
			...
			<main className="min-h-svh">
				<Content title={title} fileLocation={fileLocation} />
			</main>
			<Footer />
		</section>
	);
...
async function Content({ title, fileLocation }: { title: string; fileLocation: string }) {
	const content = await getContent(fileLocation);
			...
			<div>
				<ReactMarkdown
					components={{
						...
						h3: ({ children }) => (
							<h3 className="mb-5 mt-10 text-lg text-slate-500 dark:text-slate-400">{children}</h3>
						),
						h5: ({ children }) => (
						...
						ul: ({ children }) => (
							<ul className="list-disc pl-6 mb-4 text-slate-500 dark:text-slate-400">{children}</ul>
						),
						li: ({ children }) => (
						...
						strong: ({ children }) => (
							<strong className="font-bold text-slate-500 dark:text-slate-400">{children}</strong>
						),
					}}
				>
					{content}
